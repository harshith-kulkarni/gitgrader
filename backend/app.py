...
def fallback_analysis(repo_summary, file_analysis, commit_analysis, readme_analysis):
    ...
    try:
        ...
        if len(roadmap) > 6:
            ...
            # Add generic but still somewhat specific items
            roadmap.append(f"Add comprehensive error handling throughout the {language} codebase")
            if len(roadmap) < 5:
                roadmap.append(f"Implement logging and monitoring for the {language} application")
...
Make roadmap SPECIFIC to what's actually missing in THIS repository."""
